...
function observationSummary(data) {
  for (let i = 0; i < getResults(data).length; i++) {
    console.log(
...
function observationSummary2(data) {
  getResults(data).forEach(function (result) {
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  let arr = [];
  for (let privacy of data.results) {
    if (typeof geoPrivacy !== 'undefined' && typeof geoPrivacy === 'object') {
      if (privacy.geoprivacy === null) arr.push(privacy);
    } else if (
      typeof geoPrivacy !== 'undefined' &&
      geoPrivacy.toString().toLowerCase() === 'open'
    ) {
      if (privacy.geoprivacy === 'open') arr.push(privacy);
    } else if (
      typeof geoPrivacy !== 'undefined' &&
      geoPrivacy.toString().toLowerCase() === 'obscured'
    ) {
      if (privacy.geoprivacy === 'obscured') arr.push(privacy);
    } else if (
      typeof geoPrivacy !== 'undefined' &&
      geoPrivacy.toString().toLowerCase() === 'private'
    ) {
      if (privacy.geoprivacy === 'private') arr.push(privacy);
    }
  }
  return arr;
}
...
function transformObservation(original) {
  ...
  obj.isResearchQuality = original.quality_grade === 'research' ? true : false;
  obj.coords = original.location;
  obj.coords = obj.coords.split(',').reverse();
  obj.coords[0] = Number(obj.coords[0]);
  obj.coords[1] = Number(obj.coords[1]);

  for (let photo of original.photos) obj.photos.push(photo.url);

  obj.user = `@${original.user.login}`;
...
function transformObservations(data) {
  ...
  getResults(data).forEach(function (result) {
    arr.push(transformObservation(result));
  });
...
function transformObservations2(data) {
  ...
  getResults(data).map(function (result) {
    arr.push(transformObservation(result));
  });
...
function getObservationsById(data, ...ids) {
  let arr = new Array();
  let indexOfArray = new Array();

  ids.forEach(function (id) {
    getResults(data).find(function (result, index) {
      if (result.id === id) indexOfArray.push(index);
    });
  });

  for (let i = 0; i < indexOfArray.length; i++) {
    arr[i] = data.results[indexOfArray[i]];
  }

  if (ids.length === 1 && arr.length >= 1) return data.results[indexOfArray[0]];
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  let arr = new Array();
  getResults(data).filter(function (result) {
    if (result.positional_accuracy === options.equal) {
      arr.push(result);
    } else if (
      result.positional_accuracy >= options.min &&
      result.positional_accuracy <= options.max &&
      options.equal == null
    ) {
      arr.push(result);
    } else if (
      ...
      arr.push(result);
    } else if (options.min == null && options.max == null && options.equal == null) {
      arr = getResults(data);
    }
  });
  return arr;
...
function getTaxonPhotos(data) {
  let arr = new Array();
  let re = /https:\/\/static\.inaturalist\.org\/photos\/\d+\/square\.jpg\?\d+/;
  let obj = {
    original: '',
    square: '',
    small: '',
  ...
  getResults(data).forEach(function (result) {
    ...
    if (result.taxon === undefined) {
      arr = [];
    } else if (result.taxon.default_photo !== null) {
      url = result.taxon.default_photo.url;
      if (re.test(url)) {
        url = url.split('?');
        obj.original = url[0].replace(/square/, 'original');
        obj.square = url[0].replace(/square/, 'square');
        obj.small = url[0].replace(/square/, 'small');
        obj.medium = url[0].replace(/square/, 'medium');
        obj.large = url[0].replace(/square/, 'large');
      }
      arr.push(obj);
    }
  });
...
function getUserStats(data) {
  ...
  let total_observations = 0,
    ...
    total_species = 0,
    avr_observations = 0,
    avr_identifications = 0,
    avr_species = 0;
  obj.count = getResults(data).length;
  getResults(data).forEach(function (result) {
    total_observations += result.user.observations_count;
    total_identifications += result.user.identifications_count;
    total_species += result.user.species_count;
  });
  avr_observations = total_observations / getResults(data).length;
  avr_identifications = total_identifications / getResults(data).length;
  avr_species = total_species / getResults(data).length;
  obj.totals.observations = total_observations;
  obj.totals.identifications = total_identifications;
  obj.totals.species = total_species;
  obj.averages.observations = avr_observations;
  obj.averages.identifications = avr_identifications;
  obj.averages.species = avr_species;
