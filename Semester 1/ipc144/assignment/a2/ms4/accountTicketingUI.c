...
void menuAgent(struct AccountTicketingData *data, const struct Account *loggedingUser)
{
    ...
    // int saveticket = 0, saveaccount = 0;
    char change;
    ...
    do
    {
        ...
        if (selection == 1)
        {
            ...
            if (emptyspace == -1)
            {
                ...
                // displayAllAccountSummaryRecords(data->accounts, data->ACCOUNT_MAX_SIZE); ////test123
            ...
            else if (emptyspace != -1)
            {
                ...
                saveaccounts(data->accounts, data->ACCOUNT_MAX_SIZE, accountfilename);

                getUserLogin(&data->accounts[emptyspace].logging);

                // printf("\nname of login1 = %s\n", data->accounts[emptyspace].logging.name_login); /////test

                saveaccounts(data->accounts, data->ACCOUNT_MAX_SIZE, accountfilename);

                // printf("\nname of login2 = %s\n", data->accounts[emptyspace].logging.name_login); /////test
                // printf("\nname of account = %s\n", data->accounts[emptyspace].logging.name_account); /////test
                ...
                else if (data->accounts[found].num_account != loggedingUser->num_account)
                {
                    ...
                    else if (change == 'Y') ///////remove
                    {
                        ...
                        for (k = 0; k <= data->TICKET_MAX_SIZE; k++)
                        {
                            ...
                            {
                                ticketaccount = k;
                                if (data->tickets[ticketaccount].ticketstatus == 0)
                                {
                                    const char ticketfilename[] = "tickets_arc.txt";
                                    archivingClosedTickets(&data->tickets[ticketaccount], data->TICKET_MAX_SIZE, ticketfilename);
                                    // printf("\ncreate ticket_arc.txt\n"); ////////////////////test
                                }
                                // else
                                //     printf("\ncreate ticket_arc.txt fail"); ////////////////////test
                                ...
                                *data->tickets[ticketaccount].subjectline = '\0';
                                ...
                                // data->tickets[ticketaccount] = {"0", "0", "0", "", "0", {0}};
                            ...
                            const char ticketfilename1[] = "tickets.txt";
                            // // // printf("\nhere\n"); ///test
                            saveTickets(data->tickets, data->TICKET_MAX_SIZE, ticketfilename1);
                            // printf("\nthere\n"); ///test
                        }

                        const char accountfilename[] = "accounts_arc.txt";
                        archivingRemovedAccounts(&data->accounts[found], data->ACCOUNT_MAX_SIZE, accountfilename);
                        ...
                        const char accountfilename1[] = "accounts.txt";
                        saveaccounts(data->accounts, data->ACCOUNT_MAX_SIZE, accountfilename1);

                        // printf("\ndata account clear\n"); ////////////////////test
                        // printf("\nfound = %d", data->accounts[found].num_account); /////test
                        ...
                        // for (j = 0; j < data->ACCOUNT_MAX_SIZE; j++)
                        // {
                        //     data->accounts[j].num_account = 0;
                        //     data->accounts[j].type_account = '\0';
                        //     data->accounts[j].logging.name_account[0] = '\0';
                        //     data->accounts[j].logging.name_login[0] = '\0';
                        //     data->accounts[j].logging.password[0] = '\0';
                        //     data->accounts[j].demo.year_birth = 0;
                        //     data->accounts[j].demo.income = 0;
                        //     data->accounts[j].demo.country[0] = '\0';
                        // }

                        // printf("\nload accounts....\n");
                        // loadAccounts(data->accounts, data->ACCOUNT_MAX_SIZE);
                        // displayAllAccountSummaryRecords(data->accounts, data->ACCOUNT_MAX_SIZE); ////test123
        ...
        {
            int i = 0;
            char wylie[] = "Wylie Coyote";
            for (i = 0; i < data->ACCOUNT_MAX_SIZE; i++)
            {
                if (data->accounts[i].num_account == 53070)
                {
                    strcpy(data->accounts[i].logging.name_account, wylie);
                }
            }
        ...
        else if (selection == 10) ////////////Archive closed tickets
        {
            ...
            {
                int i = 0, num_ticketclosed = 0;
                for (i = 0; i < data->TICKET_MAX_SIZE; i++)
                {
                    if (data->tickets[i].ticketstatus == 0)
                    {
                        const char ticketfilename[] = "tickets_arc.txt";
                        archivingClosedTickets(&data->tickets[i], data->TICKET_MAX_SIZE, ticketfilename);
                        num_ticketclosed += 1;
                    }
                }
        ...
        {
            const char accountfilename[] = "accounts_arc.txt";
            int num_totalarchivedaccounts = 0;
            num_totalarchivedaccounts = totalArchivingAccounts(data->accounts, data->ACCOUNT_MAX_SIZE, accountfilename);
            printf("There are %d account(s) currently archived.\n", num_totalarchivedaccounts);
            printf("\n");
            pauseExecution();
            jump = 0;
        }
        ...
        {
            const char ticketfilename[] = "tickets_arc.txt";
            int num_totalarchivedtickets = 0, num_totalarchivedmessages = 0;

            num_totalarchivedtickets = totalArchivedTickets(data->tickets, data->TICKET_MAX_SIZE, ticketfilename);

            num_totalarchivedmessages = countArchivedMessages(data->tickets, data->TICKET_MAX_SIZE, ticketfilename);

            if (num_totalarchivedtickets != 0 && num_totalarchivedmessages != 0)
            {
                printf("There are 6 ticket(s) and a total of 54 message(s) archived.\n");
                printf("\n");
                pauseExecution();
                jump = 0;
            }
        ...
        else if (selection == 0)
        {
            ...
            // printf("   %d account saved.\n", saveaccount = loadAccounts(data->accounts, data->ACCOUNT_MAX_SIZE));

            // printf("   %d tickets saved.\n", saveticket = loadTickets(data->tickets, data->TICKET_MAX_SIZE));

            // displayAllAccountSummaryRecords(data->accounts, data->ACCOUNT_MAX_SIZE); /////test
...
{
    int selection, jump = 0, ticketFull, customeraccount, saveticket;
    ...
    do
    {
        ...
        else if (selection == 2) ////create a new ticket
        {
            ...
            if (ticketFull != 0)
            {
                ...
                //messages[data->tickets[i - 1].counter]

                data->tickets[i - 1].num_ticket = ticketFull;
                data->tickets[i - 1].accountnum_customer = record.num_account;
                data->tickets[i - 1].messages[data->tickets[i - 1].counter].type_account = record.type_account;
                strcpy(data->tickets[i - 1].messages[data->tickets[i - 1].counter].name_account, record.logging.name_account);
                data->tickets[i - 1].ticketstatus = 1;

                // ///////test
                // printf("\nbefore counter = %d\n", data->tickets[i - 1].counter);
                // printf("\nbefore num_ticket = %d\n", data->tickets[i - 1].num_ticket);
                // printf("\nbefore accountnum_customer = %d\n", data->tickets[i - 1].accountnum_customer);
                // printf("\nbefore type_account = %c\n", data->tickets[i - 1].messages->type_account);
                // printf("\nbefore name_account = %s\n", data->tickets[i - 1].messages->name_account);
                // ///////test
                ...
                // printf("\nnum_account = %d\n", record.num_account);
                // printf("\ntype = %c\n", record.type_account);
                // printf("\nname = %s\n", record.logging.name_account);

                // printf("\ndata->tickets[i - 1].counter = %d\n", data->tickets[i - 1].counter);
                // //////test
                ...
                getCString(data->tickets[i - 1].messages[data->tickets[i - 1].counter].messagedetail, 1, 150);

                data->tickets[i - 1].counter++;

                // printf("\nmessage = %s\n", data->tickets[i - 1].messages[data->tickets[i - 1].counter].messagedetail);
                // ///////test
                // printf("\nafter counter = %d\n", data->tickets[i - 1].counter);
                // printf("\nafter num_ticket = %d\n", data->tickets[i - 1].num_ticket);
                // printf("\nafter accountnum_customer = %d\n", data->tickets[i - 1].accountnum_customer);
                // printf("\nafter type_account = %c\n", data->tickets[i - 1].messages->type_account);
                // printf("\nafter name_account = %s\n", data->tickets[i - 1].messages->name_account);
                // ///////test

                const char ticketfilname[] = "tickets.txt";
                saveTickets(data->tickets, data->TICKET_MAX_SIZE, ticketfilname);
        ...
        else if (selection == 0)
        {
            ...
            printf("   %d tickets saved.", saveticket = loadTickets(data->tickets, data->TICKET_MAX_SIZE));
            puts("\n### LOGGED OUT ###");
            puts("");
            jump = 1;
        }
