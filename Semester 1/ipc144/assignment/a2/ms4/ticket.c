            ...
            else if (mod_ticket == tickets[i].num_ticket && tickets[i].ticketstatus == 1)
            {
                ...
                do
                {
                    ...
                    if (selection == 1)
                    {
                        ...
                        // saveTickets(tickets, maxsize_ticket, ticketfilname);
                    }
                    else if (selection == 2)
                    {
                ...
                else if (messagespace != -1)
                {
                    if (closeticket == 'Y' || closeticket == 'y')
                    {
                        ...
                        if (closemessage == 'Y' || closemessage == 'y')
                        {
                            ...
                            // const char ticketfilename[] = "tickets.txt";
                            // saveTickets(tickets, maxsize_ticket, ticketfilename); ////////try
        ...
        else if (selection == 3)
        {
            ...
            else if (tickets[ticketplace].ticketstatus == 0)
            {
                ...
                if (option == 'Y' || option == 'y')
                {
                    ...
                    // const char ticketfilename1[] = "tickets_arc.txt"; ////try
                    // removeActiveInArchivingFile(tickets, maxsize_ticket, ticketfilename1);
...
int saveTickets(struct Ticket *tickets, int maxSize_ticket, const char ticketfilename[])
{
    int i = 0;
    // char ch = 'a';
    FILE *fp = fopen(ticketfilename, "w");

    if (fp != NULL)
    {
        while (i < maxSize_ticket)
        {
            if (tickets[i].accountnum_customer != 0)
            {
                fprintf(fp, "%d|%d|%d|%s|%d|", tickets[i].num_ticket, tickets[i].accountnum_customer, tickets[i].ticketstatus, tickets[i].subjectline, tickets[i].counter);

                int j = 0;

                while (j < tickets[i].counter)
                {
                    fprintf(fp, "%c|%s|%s|", tickets[i].messages[j].type_account, tickets[i].messages[j].name_account, tickets[i].messages[j].messagedetail);
                    j++;
                }
...
int archivingClosedTickets(struct Ticket *tickets, int maxSize_ticket, const char ticketfilename[])
{
    // int i = 0, j = 0;
    // char ch = 'a';
    FILE *fp = fopen(ticketfilename, "a");

    if (fp != NULL)
    {

        if (tickets->accountnum_customer != 0 || tickets->ticketstatus == 0)
        {
            fprintf(fp, "%d|%d|%d|%s|%d|", tickets->num_ticket, tickets->accountnum_customer, tickets->ticketstatus, tickets->subjectline, tickets->counter);

            int j = 0;

            while (j < tickets->counter)
            {
                fprintf(fp, "%c|%s|%s|", tickets->messages->type_account, tickets->messages[j].name_account, tickets->messages[j].messagedetail);
                j++;
            }
...
int loadTickets(struct Ticket *tickets, int maxSize_ticket)
{
    ...
    int i = 0, j = 0;
    char ch = 'a';
    FILE *fp = fopen(ticketfilename, "r");

    if (fp != NULL)
    {
        while (i < maxSize_ticket && fscanf(fp, "%d|%d|%d|%30[^|]|%d|", &tickets[i].num_ticket, &tickets[i].accountnum_customer, &tickets[i].ticketstatus, tickets[i].subjectline, &tickets[i].counter) == 5)
        {
            j = 0;
            while (j < tickets[i].counter && fscanf(fp, "%c|%30[^|]|%150[^|]%c", &tickets[i].messages[j].type_account, tickets[i].messages[j].name_account, tickets[i].messages[j].messagedetail, &ch) == 4)
            {
                j++;
            }
...
int countArchivedMessages(struct Ticket *tickets, int maxSize_ticket, const char ticketfilename[])
{
    ...
    if (fp != NULL)
    {
        while (i < maxSize_ticket && fscanf(fp, "%d|%d|%d|%30[^|]|%d|", &tickets[i].num_ticket, &tickets[i].accountnum_customer, &tickets[i].ticketstatus, tickets[i].subjectline, &tickets[i].counter) == 5)
        {
            ...
            counter = counter + tickets[i].counter;
            i++;
        }
        fclose(fp);
        fp = NULL;
...
int removeActiveInArchivingFile(struct Ticket *tickets, int maxSize_ticket, const char ticketfilename[])
{
    int i = 0;
    // char ch = 'a';
    FILE *fp = fopen(ticketfilename, "w");
    ...
    if (fp != NULL)
    {
        while (i < maxSize_ticket)
        {
            if (tickets[i].accountnum_customer != 0 && tickets[i].ticketstatus != 1)
            {
                fprintf(fp, "%d|%d|%d|%s|%d|", tickets[i].num_ticket, tickets[i].accountnum_customer, tickets[i].ticketstatus, tickets[i].subjectline, tickets[i].counter);
