...
-- ID: 158000190
-- Date: NOV. 17. 2021
-- Purpose: Assignment 2 DBS311 NEE
-- ***********************
...
AS temp NUMBER;
BEGIN
    SELECT order_id INTO temp FROM order_items WHERE order_id = orderId;

    INSERT INTO order_items
    (order_id, item_id, product_id, quantity, unit_price)
    VALUES
    (orderId, itemId, productId, quantity1, price);
    dbms_output.put_line('Successfully adding order item.');
...
BEGIN
    ...
    LOOP
    round := round + 1;
    FETCH order_cursor INTO o_id, c_id, item_id, product_id, quantity, unit_price, total;
    ...
    dbms_output.put_line('  Item ID: '|| item_id || ', Product ID: '|| product_id || ', Quantity: '|| quantity || ', Price: '|| unit_price);
...
out1 NUMBER;
out2 products.list_price%TYPE;
out3 NUMBER;

BEGIN
    CASE task
    WHEN 1 THEN find_customer(parm1, out1);
    WHEN 2 THEN find_product(parm1, out2);
    WHEN 3 THEN add_order(parm1, out3);
